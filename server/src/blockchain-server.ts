...
export class BlockchainServer extends MessageServer<Message> {
    ...
    private handleGetLongestChainRequest(requester: WebSocket, message: Message): void {
        if (this.clientIsNotAlone) {
            this.receivedMessagesAwaitingResponse.set(message.correlationId, requester);
    ...
    private handleGetLongestChainResponse(sender: WebSocket, message: Message): void {
        if (this.receivedMessagesAwaitingResponse.has(message.correlationId)) {
            const requester = this.receivedMessagesAwaitingResponse.get(message.correlationId);
    ...
    private handleAddTransactionsRequest(requester: WebSocket, message: Message): void {
        this.broadcastExcept(requester, message);
    }

    private handleNewBlockAnnouncement(requester: WebSocket, message: Message): void {
        this.broadcastExcept(requester, message);
    }
